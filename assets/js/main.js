...
async function loadPhotosSafe() {
  ...
  try {
    ...
    // ✅ Content-Type 检查（关键防爆）
    const contentType = res.headers.get('content-type') || '';
    if (!contentType.includes('application/json')) {
      // 如果是 HTML（<!DOCTYPE>），直接报错
      if (text.trim().startsWith('<!DOCTYPE')) {
        throw new Error('代理返回了 HTML，不是 JSON（可能代理失效或需授权）');
      }
    ...
    try {
      ...
      throw new Error('JSON 解析失败，返回内容异常');
    }

    // ✅ 数据结构检查
    if (!Array.isArray(files)) {
      throw new Error('API 返回格式错误（非数组）');
    }
    ...
    bar.style.display = 'none';
    console.error('[loadPhotosSafe]', err);
  }
}
...
function encodeKey(k) { return btoa(AI_ENCRYPT_KEY + '|' + k); }
function decodeKey(e) { try { return atob(e).replace(AI_ENCRYPT_KEY + '|', ''); } catch { return ''; } }
function saveAIConfig(cfg) { localStorage.setItem('ai_config', btoa(JSON.stringify(cfg))); }
function loadAIConfig() {
  const s = localStorage.getItem('ai_config');
  if (!s) return null;
  try { const cfg = JSON.parse(atob(s)); return { ...cfg, key: decodeKey(cfg.key) }; } catch { return null; }
}
